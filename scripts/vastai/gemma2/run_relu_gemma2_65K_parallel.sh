...
# Launch 3 ReLU SAE sweeps in parallel on GPUs 1-3

# Get the project root directory (two levels up from this script)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../.." && pwd)"

SPARSITY_COEFFS=(23 24 25)
CONFIGS=(
    "configs/gemma2-2b/sweep/relu_sweeps/65K_relu_coeff_23.yaml"
    "configs/gemma2-2b/sweep/relu_sweeps/65K_relu_coeff_24.yaml"
    "configs/gemma2-2b/sweep/relu_sweeps/65K_relu_coeff_25.yaml"
)

for i in {0..2}; do
    SESSION_NAME="relu_sparsity_coeff_${SPARSITY_COEFFS[$i]}"
    GPU=$((i + 1))
    CONFIG="${CONFIGS[$i]}"
