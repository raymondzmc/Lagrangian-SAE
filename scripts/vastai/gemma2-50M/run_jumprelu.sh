...
tmux send-keys -t $SESSION "cd $WORKDIR && $CONDA_CMD && CUDA_VISIBLE_DEVICES=6 python run_sweep.py --base_config configs/gemma2-2b-50M/gemma2-jumprelu.yaml --sweep_config configs/gemma2-2b-50M/sweep/16K_l0_sweep.yaml" C-m

# Split horizontally for pane 1: JumpReLU 65K (GPU 7)
tmux split-window -h -t $SESSION
tmux send-keys -t $SESSION "cd $WORKDIR && $CONDA_CMD && CUDA_VISIBLE_DEVICES=7 python run_sweep.py --base_config configs/gemma2-2b-50M/gemma2-jumprelu.yaml --sweep_config configs/gemma2-2b-50M/sweep/65K_l0_sweep.yaml" C-m
